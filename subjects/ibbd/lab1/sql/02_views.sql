...
SELECT o.order_id, c.full_name AS customer_name, c.phone,
 i.inventory_no, i.name AS instrument_name, t.name AS instrument_type,
...
-- Сначала независимая агрегация работ и оплат: нет умножения сумм.
CREATE VIEW v_reception_balances AS
WITH costs AS (
 SELECT order_id, sum(agreed_price_kopecks) AS total
 FROM order_work WHERE status<>'cancelled' GROUP BY order_id
), paid AS (
...
SELECT o.order_id, c.full_name AS customer_name,
 ...
 coalesce(costs.total,0)-coalesce(paid.total,0) AS balance_kopecks
FROM restoration_order o
JOIN instrument i ON i.instrument_id=o.instrument_id
JOIN customer c ON c.customer_id=i.customer_id
...
-- Реставратор: очередь работ без контактов заказчика и финансов.
CREATE VIEW v_restorer_tasks AS
SELECT w.employee_id, e.full_name AS restorer_name,
 w.order_id, w.line_no, i.inventory_no, i.name AS instrument_name,
 s.name AS service_name, o.due_on, w.status AS work_status
FROM order_work w
...
SELECT x.inspection_id, x.order_id, i.inventory_no, i.name AS instrument_name,
 x.inspected_on, e.employee_id, e.full_name AS inspector_name, x.conclusion
FROM inspection x
